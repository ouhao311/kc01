...
class Page {
    ...
    public function __construct() {
        ...
        $this->pre_home = L("first_page");
        $this->pre_last = L("last_page");
        $this->pre_page = L("pre_page");
        $this->next_page = L("next_page");
		$this->pre_page2="<";
		$this->next_page2= ">";
        ...
        switch ($this->style) {
            ...
            case '5':
                ...
                if ($this->getNowPage() <= 1) {
                    ...
                    $html_page.= "<li class='page-item'><a class='page-link' href='". $this->setShopPseudoStaticPageUrl($this->getNowPage() - 1) ."' >上一页</a></li>";
                }
                $html_page.= $this->getNowBar();
                if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0) {
                    $html_page.= '<li class="page-item disabled"><span class="page-link">下一页</span></li>';
                } else {
                    $html_page.= "<li class='page-item'><a class='page-link' href='". $this->setShopPseudoStaticPageUrl($this->getNowPage() + 1) ."' >下一页</a></li>"; 
                }  
				$html_page.= '';  
                break;
    ...
    private function getNowBar() {
        ...
        if ($end < $this->getTotalPage()) {
            $result.= '<li class="page-item"><a>...</a></li>' ;
        }
        return $result;
    }
    private function setPageHtml($page_name, $page) {
        if ($this->getNowPage() == $page) {
            $result = "<li class='active page-item disabled' ><span class='page-link'>". $page .'</span></li>';
        } else if (in_array($this->style, array(
            4,
            5
        ))) {
            $result = "<li class='page-item'><a class='page-link' href='" . $this->setShopPseudoStaticPageUrl($page) . "'>" . $page_name . "</a></li>" ;
        } else {
            $result = "<li class='page-item'><a class='page-link' href='" . $this->page_url . $page . "'>" . $page_name  . "</a></li>" ;
        }
        return $result;
    }
    ...
    private function setShopPseudoStaticPageUrl($page) {
        $param = $_GET;
        $url = ($param["url"] == "" ? "index" : $param["url"]);
        unset($param["url"]);
        $do = ($param["do"] == "" ? "index" : $param["do"]);
        unset($param["do"]);
        $param[$this->page_name] = $page;
        return url($url, $do, $param);
    }
}
defined('SSZCMS') || exit('Access Denied');
?>