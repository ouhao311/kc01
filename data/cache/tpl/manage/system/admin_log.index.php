...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <div class="subject">
        <h3><?php echo $lang['hx_admin_log'];?></h3>
        <h5><?php echo $lang['hx_admin_log_subhead'];?></h5>
      </div>
    </div>
  </div>
  <!-- 操作说明 -->
  ...
  <div class="ncap-search-bar">
    ...
    <form method="get" name="formSearch" id="formSearch">
      ...
      <div class="bottom"> <a href="javascript:void(0);" id="ncsubmit" class="ncap-btn ncap-btn-green mr5">提交查询</a><a href="javascript:void(0);" id="ncreset" class="ncap-btn ncap-btn-orange" title="撤销查询结果，还原列表项所有内容"><i class="fa fa-retweet"></i><?php echo $lang['hx_cancel_search'];?></a></div>
    </form>
  </div>
</div>
...
$('#query_start_date').datepicker();
    ...
    $('#ncsubmit').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?url=admin_log&do=get_xml&'+$("#formSearch").serialize(),query:'',qtype:''}).flexReload();
    });
    // 高级搜索重置
    $('#ncreset').click(function(){
        $("#flexigrid").flexOptions({url: 'index.php?url=admin_log&do=get_xml'}).flexReload();
        $("#formSearch")[0].reset();
    });
    $("#flexigrid").flexigrid({
        url: 'index.php?url=admin_log&do=get_xml',
        colModel : [
...
function fg_operate(name, grid) {
    ...
    $.ajax({
            type: "GET",
            dataType: "json",
            url: "index.php?url=admin_log&do=list_del",
            data: "type=ago",
...
function fg_csv(ids) {
    id = ids.join(',');
    window.location.href = $("#flexigrid").flexSimpleSearchQueryString()+'&do=export_step1&id=' + id;
}
