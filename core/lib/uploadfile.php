...
//				}
		...
		if(@move_uploaded_file($this->upload_file['tmp_name'],HX_UPLOAD.DS.$this->save_path.DS.$this->file_name)){
			//产生缩略图
			if ($ifresize){
				$resizeImage	= new ResizeImage();
				$save_path = rtrim(HX_UPLOAD.DS.$this->save_path,'/');
				for ($i=0;$i<count($thumb_width);$i++){
			...
			//删除原图
			if ($this->ifremove && is_file(HX_UPLOAD.DS.$this->save_path.DS.$this->file_name)) {
				@unlink(HX_UPLOAD.DS.$this->save_path.DS.$this->file_name);
			}
			return true;
		}else {
...
//				$resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling);
	...
	public function setPath(){
		 ...
		 * 判断目录是否存在，如果不存在 则生成
		 */
		if (!is_dir(HX_UPLOAD.DS.$this->default_dir)){
			$dir = $this->default_dir;
			$dir_array = explode(DS,$dir);
			$tmp_base_path = HX_UPLOAD;
		...
		//设置权限
		@chmod(HX_UPLOAD.DS.$this->default_dir,0755);

		//判断文件夹是否可写
        if(!is_writable(HX_UPLOAD.DS.$this->default_dir)) {
