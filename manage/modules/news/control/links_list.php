...
class links_listControl extends SystemControl{
    public $name;
    ...
    public function get_xmlDo(){ 
        ...
        foreach ($list as $k => $v) {
			...
			$links_class=getSinglePas($table='attribute','links_class',$v['links_class'],'title');
			$list['links_class'] = "<span title='".$links_class."'>".$links_class."</span>"; 
			/* if($v['pic']){
    ...
    public function addDo(){
        ...
        if (chksubmit()){
            ...
            $obj_validate->validateparam = array(
                array("input"=>$_POST["title"], "require"=>"true", "message"=>"请填写友情链接标题"), 
            );
            ...
            if ($error != ''){
                ...
                $data['links_class']      = trim($_POST['links_class']);
                $data['edittime']  = time();
    ...
    public function setDo() {
        ...
        $result =M('links_list')->where($condition)->update($data);
        if($result){
