...
//布局换色设置
    ...
    $('.nav-tabs').each(function(){
        $(this).find('dl > dt > a').each(function(i){
            $(this).parent().next().css('top', (-55)*i + 'px');
            $(this).click(function(){
    ...
    $('div[data-param^="map-"]').find('i').click(function(){
        ...
        if ($this.parent().hasClass('selected')) {
            $.getJSON('index.php?url=common&do=common_operations', {type : 'del', value : _value}, function(data){
                if (data) {
            ...
            $.getJSON('index.php?url=common&do=common_operations', {type : 'add', value : _value}, function(data){
                if (data) {
    ...
    if ($.cookie('commonPendingMatters') == null) {
        $.ajax({
                url : 'index.php?url=common&do=ajax_pending_matters',
                async : false,
...
function openItem(param) {
    ...
    $('#workspace').attr('src', ADMIN_SITE_URL + '/modules/' + data_str[0] + '/index.php?url=' + data_str[1]);
