...
$(function(){
    $("#flexigrid").flexigrid({
        url: 'index.php?url=area&do=get_xml',
        colModel : [
...
function fg_operate(name, grid) {
    if (name == 'add') {
        window.location.href = 'index.php?url=area&do=add&parent_id='+cur_parent_id;
    }else if (name == 'delete') {
...
$("#flexigrid").flexOptions({url: 'index.php?url=area&do=get_xml&parent_id='+area_id}).flexReload();
...
$("#flexigrid").flexOptions({url: 'index.php?url=area&do=get_xml&parent_id='+his_parent_ids[his_parent_ids.length-1]}).flexReload();
...
<script type="text/javascript">
  function fg_set(id,branch) { 
    $.ajax({
      type: "GET",
      dataType: "json",
      url: "index.php?url=area&do=set",
      data: "area_id="+id+'&branch='+branch,
      success: function(data){
        if (data.state){
          $("#flexigrid").flexReload();
        } else {
