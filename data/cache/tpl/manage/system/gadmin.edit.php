...
<div class="page">
    ...
    <div class="ncap-form-all">
      ...
      <dl class="row">
        ...
        <dd class="opt nobg nopd nobd nobs">
          ...
          <div class="ncap-account-container">
            <h4>
             <input class="checkbox" type="checkbox" value="<?php echo $k;?>" name="permission[<?php echo $key?>][<?php echo $k;?>][<?php echo $k;?>]" <?php if (@in_array($k, $ginfo['limits'][$key][$k])) { echo 'checked';}?>>
                <?php echo $v["name"];?>
             </h4>
 ...
 <?php if(@in_array('all', $czxx)) { ?>
 
 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][view]" <?php if ($ginfo['limits'][$key][$k]["view"]) { echo 'checked';}?>>
   查看
  
</li>
<li>
 
 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][add]" <?php if ($ginfo['limits'][$key][$k]["add"]) { echo 'checked';}?>>
   添加
</li>

  <li>

 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][edit]" <?php if ($ginfo['limits'][$key][$k]["edit"]) { echo 'checked';}?> >
   修改
</li>

  <li>

 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][del]" <?php if ($ginfo['limits'][$key][$k]["del"]) { echo 'checked';}?> >
   删除
</li>
  <li>

 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][shenhe]" <?php if ($ginfo['limits'][$key][$k]["shenhe"]) { echo 'checked';}?> >
   审核
</li>
 ...
 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][progress]" <?php if ($ginfo['limits'][$key][$k]["progress"]) { echo 'checked';}?> >
   进度查询
</li>
 ...
 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][topshenhe]" <?php if ($ginfo['limits'][$key][$k]["topshenhe"]) { echo 'checked';}?> >
   高级审核
</li>
   
 <li>    

 <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][all]" <?php if ($ginfo['limits'][$key][$k]["all"]) { echo 'checked';}?> >
   全部

</li>
 <?php } else { ?>
  <?php if(@in_array('view', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][view]" <?php if ($ginfo['limits'][$key][$k]["view"]) { echo 'checked';}?>>
                查看
<?php } ?>
             </li>
             <li>
   <?php if(@in_array('add', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][add]" <?php if ($ginfo['limits'][$key][$k]["add"]) { echo 'checked';}?>>
                添加<?php } ?>
             </li>
             
               <li>
     <?php if(@in_array('edit', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][edit]" <?php if ($ginfo['limits'][$key][$k]["edit"]) { echo 'checked';}?> >
                修改<?php } ?>
             </li>
             
               <li>
     <?php if(@in_array('del', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][del]" <?php if ($ginfo['limits'][$key][$k]["del"]) { echo 'checked';}?> >
                删除<?php } ?>
             </li>
               <li>
    <?php if(@in_array('shenhe', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][shenhe]" <?php if ($ginfo['limits'][$key][$k]["shenhe"]) { echo 'checked';}?> >
                审核<?php } ?>
             </li>
             
                <li>
    <?php if(@in_array('progress', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][progress]" <?php if ($ginfo['limits'][$key][$k]["progress"]) { echo 'checked';}?> >
                进度查询<?php } ?>
             </li>
             
               <li>
    <?php if(@in_array('topshenhe', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][topshenhe]" <?php if ($ginfo['limits'][$key][$k]["topshenhe"]) { echo 'checked';}?> >
                高级审核<?php } ?>
             </li>
               ...
               <li>
     <?php if(@in_array('pay', $czxx)) { ?>
              <input class="checkbox" type="checkbox" value="1" name="permission[<?php echo $key?>][<?php echo $k;?>][pay]" <?php if ($ginfo['limits'][$key][$k]["pay"]) { echo 'checked';}?> >
                缴费<?php } ?>
             </li>
