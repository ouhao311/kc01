...
class volunteer_listControl extends SystemControl{
	public $name;
	...
	// print_r($condition);exit;
			...
			foreach ($list as $k => $v) {
				...
				$condition2 	= array();
				$condition2['volunteerid'] = intval($v['id']);
    ...
    public function reviewDo(){ 
		...
		$volunteerinfo=getTableInfohanett($info['volunteerid'],'volunteer_list');
		if (chksubmit()){ 
			$data = array();
			...
			if ($result){
				$this->log('任务数据审核'.'['.$volunteerinfo['name'].']',null);
				if($data['revstatus']==1){
					if($info['transaction']>0){
						$memberinfo=getTableInfohanett($info['transaction'],'member');
						//更新积分
						M('member')->where(array('id'=>$info['transaction']))->update(array('bank'=>$memberinfo['bank']+2)); //C('bank')
						...
						M("member_bank")->insert($data_record);
					}
				}
				echo "<script>window.parent.layer.closeAll();window.parent.$('#flexigrid').flexReload();window.parent.layer.msg('操作成功');</script>";
				exit();
    ...
    public function topreviewDo(){ 
		...
		$mid=$_SESSION['member_id'];
		$model=M('volunteer_detail_list');
		$condition['volunteerid'] = intval($_GET['volunteerid']);
			...
			if ($result){
				...
				if($data['revstatus']==2){
					if($info['transaction']>0){
						...
						$data_record['addtime']=time();
						$data_record['source']=1;
						$data_record['intro']='任务数据'.'['.$volunteerinfo['name'].']审核成功奖励2积分';
						M("member_bank")->insert($data_record);
					}
				}
				echo "<script>window.parent.layer.closeAll();window.parent.$('#flexigrid').flexReload();window.parent.layer.msg('操作成功');</script>";
