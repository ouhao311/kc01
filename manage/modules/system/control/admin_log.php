...
class admin_logControl extends SystemControl{
    ...
    public function list_delDo(){
        ...
        if (!M('admin_log')->where($condition)->delete()){
            $this->log(L('hx_del,hx_admin_log'),0);
            exit(json_encode(array('state'=>false,'msg'=>'删除失败')));
        }else{
            $this->log(L('hx_del,hx_admin_log'),1);
     ...
     * 导出第一步
     */
    public function export_step1Do(){
        $model = M('admin_log');
        $condition = array();
        ...
        if (!is_numeric($_GET['curpage'])){
            ...
            if ($count > self::EXPORT_SIZE ){   //显示下载链接
				...
				$output['list']=$array;
				$output['murl']='index.php?url=admin_log&do=list';
				include T('export.excel');
    ...
    private function createExcel($data = array()){
        ...
        $excel_obj->addWorksheet($excel_obj->charset(L('hx_admin_log'),CHARSET));
        $excel_obj->generateXML($excel_obj->charset(L('hx_admin_log'),CHARSET).$_GET['curpage'].'-'.date('Y-m-d-H',time()));
    }
