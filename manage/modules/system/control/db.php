...
class dbControl extends SystemControl{
	...
	public function dbDo(){
		...
		if ($_POST['form_submit'] == 'ok'){
			 ...
			 * 验证文件夹是否重复
			 */
			if (is_dir(HX_DATA.DS.'sql_back'.DS.$_POST['backup_name'])) {
			...
			showMessage($lang['db_index_backup_to_wait'],'index.php?url=db&do=db&step=1');
		}
		/**
		 * 数据库备份步骤
		 */
		if (intval($_GET['step']) >= 1){
			...
			if (
				...
				$url = array(
					array(
						'url'=>'index.php?url=db&do=db&step='.(intval($_GET['step'])+1),
		...
		$output['back_dir']=$back_dir;
		$output['table_list']=$table_list;
		include T('db.index');
	...
	public function db_restoreDo(){
		...
		if ($_POST['form_submit'] == 'ok'){
			if (!empty($_POST['dir_name']) && is_array($_POST['dir_name'])){
				$dir = HX_DATA.DS.'sql_back';
				foreach ($_POST['dir_name'] as $k => $v){
					if (file_exists(HX_DATA.DS.'sql_back'.DS.$v)){
						$file_list  = array();
		...
		$tmp_list = readDirList(HX_DATA.DS.'sql_back');
		/**
		 * 整理内容
		 */
		$dir_list = array();
		if (is_array($tmp_list)){
			foreach ($tmp_list as $k => $v){
				$dir_list[$k]['name'] = $v;
				$dir_list[$k]['make_time'] = date('Y-m-d H:i:s',filemtime(HX_DATA.DS.'sql_back'.DS.$v));
				$dir_list[$k]['size'] = number_format((getDirSize(HX_DATA.DS.'sql_back'.DS.$v)/1024),2).'KB';
				$dir_list[$k]['file_num'] = count(glob(HX_DATA.DS.'sql_back'.DS.$v.DS."*.sql"));
			}
		}

		$output['dir_list']=$dir_list;
		include T('db.restore');
	 ...
	 * 数据库备份导入
	 */
	public function db_importDo(){
		$lang 	= Language::getLangContent();
		if ($_GET['dir_name'] != '' && file_exists(HX_DATA.DS.'sql_back'.DS.$_GET['dir_name'])){
			$model_db = M('db');
			...
			if ($result == 'succ'){
				$url = array(
					array(
						'url'=>'index.php?url=db&do=db_restore',
			...
			if ($result == 'continue'){
				$url = array(
					array(
						'url'=>'index.php?url=db&do=db_import&dir_name='.$_GET['dir_name'].'&step='.(intval($_GET['step'])+1),
	 ...
	 * 数据库 删除
	 */
	public function db_delDo(){
		$lang 	= Language::getLangContent();
		if ($_GET['dir_name'] != '' && file_exists(HX_DATA.DS.'sql_back'.DS.$_GET['dir_name'])){
			$dir = HX_DATA.DS.'sql_back'.DS.$_GET['dir_name'];
			$file_list = array();
			 ...
			 * 删除目录
			 */
			@rmdir(HX_DATA.DS.'sql_back'.DS.$_GET['dir_name']);
	...
	public function ajaxDo(){
		switch ($_GET['branch']){
			 ...
			 */
			case 'db_file':
				if ($_GET['dir_name'] != '.' && $_GET['dir_name'] != '..' && is_dir(HX_DATA.DS.'sql_back'.DS.$_GET['dir_name'])){
					$dir = HX_DATA.DS.'sql_back'.DS.$_GET['dir_name'];
					$tmp = array();
