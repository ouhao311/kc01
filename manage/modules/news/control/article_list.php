...
class article_listControl extends SystemControl{
	public $name;
    ...
    public function get_xmlDo(){
        ...
        foreach ($list as $k => $v) {
			...
			if($v['pid']){
				$article_list=getTableInfohanett($v['pid'],'article_class');
				$list['pid'] = "<span title='".$article_list['title']."'>".$article_list['title']."</span>";
			}else{
				$list['pid'] ="暂无分类";
			}  
    ...
    public function addDo(){
        ...
        if (chksubmit()){
			...
			if ($result){
				$this->log('资讯列表添加'.'['.$data['title'].']',null);
