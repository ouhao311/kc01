...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title"><a class="back" href="index.php?url=attribute" title="返回列表"><i class="fa fa-arrow-circle-o-left"></i></a>
      <div class="subject">
        <h3><?php echo $pidname;?>的次级属性管理 - 添加/编辑 - <?php echo $info['title'];?></h3>
        <h5><?php echo $pidname;?>的次级属性索引与管理</h5>
      </div>
    </div>
  </div>
  <form id="article_form" enctype="multipart/form-data" method="post">
    ...
    <div class="ncap-form-default">
      <dl class="row">
        <dt class="tit">
          <label for="title"><em>*</em>次级属性名称</label>
        </dt>
        <dd class="opt">
      ...
      <dl class="row">
        <dt class="tit">
          <label for="pid"><em>*</em>属性父级分类</label>
        </dt>
        <dd class="opt">
         <?php echo getselectsx('attribute','pid',$pid,1);?>
          <span class="err"></span>
          <p class="notic"></p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label for="title">次级属性排序</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $info['rank'];?>" name="rank" id="rank" class="input-txt">
          <span class="err"></span>
          <p class="notic">次级属性显示顺序，数值越大越靠前</p>
        </dd>
      <dl class="row">
        <dt class="tit">
          <label for="title">次级属性value值</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $info['value'];?>" name="value" id="value" class="input-txt">
          <span class="err"></span>
          <p class="notic">例：1,2,3...在同一个顶级分类下的次级属性value值不可以重复！</p>
        </dd>
      </dl>
      <dl class="row">
        <dt class="tit">
          <label for="title"><em>*</em>次级属性code值</label>
        </dt>
        <dd class="opt">
          <input type="text" value="<?php echo $info['code'];?>" name="code" id="code" class="input-txt">
          <span class="err"></span>
          <p class="notic">次级属性code值和它对应的顶级属性value值相等，请谨慎操作！</p>
        </dd>
      </dl>
...
$(document).ready(function(){
  $('#article_form').validate({
   ...
   messages : {
      title : {
                required : '<i class="fa fa-exclamation-circle"></i>次级属性名称不能为空！'
            },
      pid : {
