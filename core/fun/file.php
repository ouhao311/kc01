...
function file_copy($from, $to) {
	...
	if(is_file($to) && HX_CHMOD) @chmod($to, HX_CHMOD);
	if(strpos($from, HX_PATH) !== false) $from = str_replace(HX_PATH, HX_ROOT.'/', $from);
	if(@copy($from, $to)) {
		if(HX_CHMOD) @chmod($to, HX_CHMOD);
		return true;
	} else {
		return false;
	}
...
function file_put($filename, $data) {
	...
	if(@$fp = fopen($filename, 'wb')) {
		...
		flock($fp, LOCK_UN);
		fclose($fp);
		if(HX_CHMOD) @chmod($filename, HX_CHMOD);
		return $len;
	} else {
		return false;
	}
...
if(is_dir($path)) return true;
	...
	if(HX_CACHE != HX_ROOT.'/cache' && strpos($path, HX_CACHE) !== false) {
		$dir = str_replace(HX_CACHE.'/', '', $path);
		$dir = dir_path($dir);
		$temp = explode('/', $dir);
		$cur_dir = HX_CACHE.'/';
		$max = count($temp) - 1;
		for($i = 0; $i < $max; $i++) {
			...
			@mkdir($cur_dir);
			if(HX_CHMOD) @chmod($cur_dir, HX_CHMOD);
			if(!is_file($cur_dir.'/index.html') && !is_file($cur_dir.'/index.php')) file_copy(HX_ROOT.'/cache/index.html', $cur_dir.'/index.html');
		}
	} else {
		$idx = strpos($path, '/cache/') !== false ? true : false;
		$dir = str_replace(HX_ROOT.'/', '', $path);
		$dir = dir_path($dir);
		$temp = explode('/', $dir);
		$cur_dir = HX_ROOT.'/';
		$max = count($temp) - 1;
		for($i = 0; $i < $max; $i++) {
			...
			@mkdir($cur_dir);
			if(HX_CHMOD) @chmod($cur_dir, HX_CHMOD);
			if($idx && !is_file($cur_dir.'/index.html') && !is_file($cur_dir.'/index.php')) file_copy(HX_ROOT.'/cache/index.html', $cur_dir.'/index.html');
		}
	}
	return is_dir($path);
}
...
function dir_copy($fromdir, $todir) {
	...
	foreach($list as $v) {
		...
		if(is_file($path) && !is_writable($path)) {
			if(HX_CHMOD) @chmod($path, HX_CHMOD);
		}
		if(is_dir($v)) {
		    dir_copy($v, $path);
		} else {
...
function dir_delete($dir) {
	...
	$dirs = array(HX_ROOT.'/manage/', HX_ROOT.'/api/',  HX_ROOT.'/include/', HX_ROOT.'/lang/', HX_ROOT.'/member/', HX_ROOT.'/module/', HX_ROOT.'/extend/', HX_ROOT.'/skin/', HX_ROOT.'/template/', HX_ROOT.'/wap/');
