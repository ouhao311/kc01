...
class Model{
    ...
    public function tableInfo($table)
    {
        ...
        //只取主键,find(2)等自动匹配主键时使用
            ...
            if (file_exists(HX_DATA.'/cache/fields/_pk.php')){
                $this->fields = require(HX_DATA.'/cache/fields/_pk.php');
            } else {
                $_pk_array = self::fetchTablePkArray();

                write_file(HX_DATA.'/cache/fields/_pk.php', $_pk_array);
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
                ...
                }elseif(preg_match('/DATE/i',$val[0])){

                    $data = is_string($val[1])? explode(',',$val[1]):$val[1];
                    if($data[0] && $data[1]) {
                        $whereStr .=  " (".$key." BETWEEN '".$data[0]."' AND '".$data[1]." 23:59:59'"." )";
                    } elseif ($data[0]) {
                        $whereStr .= $key.' '.$this->comparison['egt'].' '."'".$data[0]."'";
                    } elseif ($data[1]) {
                        $whereStr .= $key.' '.$this->comparison['elt'].' '."'".$data[1].' 23:59:59'."'";
                    }
